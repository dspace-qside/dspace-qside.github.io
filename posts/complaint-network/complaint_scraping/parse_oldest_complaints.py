...
This data was downloaded using the following steps:
- In the search bar, each possible first name was searched painstakingly
...
azlist = ['A-D', 'E-H', 'I-J','K-L','M-Q','R-S','T-Z']
# Read in the PDF and save x and y locations of text
for azportion in azlist:
    in_f_name = "CRA Public Profile Card "+ azportion + '.pdf'
    out_f_name= 'old_old_complaints_' + azportion +'.csv'

    df = extract_and_format(in_f_name, 0)

    # determine the name columns by x location
    df['is_name'] = df['x'] < 100

    # determine case number (starting EVT-) with string match 
    df['is_num'] = df['text'].str.contains(r'\d\d-\d+')
    ...
    if azportion == 'A-D':
        ...
        dffinal = pd.concat([dffinal, df2], ignore_index=True)

dffinal.to_csv('old_old_complaints.csv')