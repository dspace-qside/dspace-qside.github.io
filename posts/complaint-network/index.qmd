...
language: 
  title-block-modified: "DATA LAST UPDATED"
description: ' '
image: network.png
...
In 2012, the MPD overhauled their complaint system for increased accountability and to add internal oversight structures. The [Office of Police Conduct Review (OPCR)](https://www.minneapolismn.gov/government/departments/civil-rights/opcr/) was established by local ordinance to address complaints and cases of misconduct. Under this system, civilians must submit complaints to OPCR within 270 days of alleged misconduct. After an evidence-gathering period, the OPCR either performs a full investigation within 180 days, refers the complaint to MPD when training is needed over discipline, or dismisses the case @roth2023transforming. There are a number of decision-making steps, each from various participants and with waiting times: the OPCR director’s decision, a panel review, and the police chief’s review and decision. Finally, the case is marked closed, either with Discipline or No Discipline. If the complaint was sustained and resulted in disciplinary action, information such as location and violation type is made public. Otherwise, a case is marked No Discipline and no further details are released. Importantly, the No Discipline category includes **both unfounded/exonerated complaints and sustained complaints that resulted in non-disciplinary action**.[^2] For more information about the complaint filing and decision-making process, refer to the OPCR website @opcrprocess. 

[^2]: We included the "Closed- No Discipline" category in our final data because this category includes complaints that *were* substantiated but resulted in non-disciplinary corrective actions such as coaching or training. However, we have no way to differentiate between these and cases that were exonerated, which is a key limitation of this work. 
...
#how many officers do not appear in the complaint dataset?
num_off_no_compl <- length(which(!(officer_names_roster %in%
                                     new_complaints$format_name)))
...
#add all officers with no complaints for visualization
#no complaints officers
off_no_comp_df <- cbind(officer_names_roster[which(!(officer_names_roster %in% 
                                              new_complaints$format_name))],
                        rep(0, num_off_no_compl)) %>% as.data.frame()
colnames(off_no_comp_df) <- colnames(comp_per_off)
off_no_comp_df$n <- as.numeric(off_no_comp_df$n)

#combine data for officers with complaints and with 0 complaints
comp_per_off_with_zeros <- bind_rows(comp_per_off,
                                 off_no_comp_df)

#distribution of comp per officers
p1 <- ggplot(data=comp_per_off_with_zeros) + 
  geom_histogram(aes(x=n,
                     text = paste("complaints:", scales::number(n, accuracy = 1)))) +
...
ggplotly(p1, tooltip = c("count", "text"))
```
...
p2 <- ggplot(data=off_per_comp) + 
  geom_histogram(aes(x=n,
                     text = paste("officers:", scales::number(n, accuracy = 1)))) +
...
```{r, echo = F, include = T, message = F}
ggplotly(p2, tooltip = c("count", "text"))
```
...
**This data is updated as of May 8, 2025. We plan to update the data used in this app.**
